...
interface Props {
  id: string
  /** Show the action bar. Off inside the preview iframe, which has its own. */
  showToolbar?: boolean
}

/**
 * The rendered A4 invoice. Shared by the full page (with toolbar, inside the
 * dashboard shell) and by the preview route, which renders the document alone
 * so the iframe shows a clean sheet of paper instead of a nested dashboard.
 */
export default async function InvoiceDocument({ id, showToolbar = false }: Props) {
      ...
      <style>{`
        ...
        @media screen and (max-width: 860px) {
          ...
          .invoice-scale > .page {
            ...
            margin-bottom: calc((100vw - 24px) / 794 * 1123px - 1123px);
          }
        }
      `}</style>
      ...
      <div className="no-print flex flex-wrap items-center gap-2 px-4 sm:px-6 pt-4 pb-3 bg-white border-b border-slate-200 sticky top-0 z-10">
        ...
        {hasPayments && !isCancelled && (
          <DepositEmailButton
            invoiceId={inv.id}
            captureInvoice
            sentAt={inv.deposit_email_sent_at ?? null}
          />
        )}
